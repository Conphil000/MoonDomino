...
class DOMINO:
    ...
    def TRUMP(self,trump=None):
        return (self._s1 == trump) or (self._s2 == trump)
    ...
    def BEAT(self,): # do i need for strat i guess??
        beats = [1,2,3,4,5,6]
        if self.__ACE():
...
if __name__ == '__main__':
    ...
    assert sum([i.TRUMP(4) for i in hand]) == 0
    assert sum([i.TRUMP(2) for i in hand]) == 2
